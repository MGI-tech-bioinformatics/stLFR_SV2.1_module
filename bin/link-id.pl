...
die "Usage: $0 <cluster file> <link id file> <low depth> <exclude depth> <low depth share> <exclude depth share> <gap size> <bin size> <N merge bins>\n" if @ARGV != 9;

my ($infile,$outfile,$low,$ex,$lows,$exs,$gap,$bin,$Nmerge)=@ARGV;
...
if(1){
	...
	foreach my $ref(@sv){
		...
		push @new_sv,[($sin{$result[2]}{$result[0]}{$result[1]}{"sid"},$sin{$result[5]}{$result[3]}{$result[4]}{"sid"},@result)];
	}
...
if(1){
	...
	for (my $i=0;$i<=$#new_sin;$i++){
		...
		print OUT "$i\t$chr\t$pos\t$ori\t$count\t$line\n";
	}
	close OUT;
}
...
if(1){
	...
	#ida idb chra posa oria chrb posb orib count id bar
	# my $R = Statistics::R->new();
	# my $cmd=qq{
# library(MASS)
...
# };
	...
	foreach my $ref (@sv){
		...
		my $sup1=$infoarray[$ref->[0]];
		my $sup2=$infoarray[$ref->[1]];
		my $sup3=$ref->[8];
		...
		my $mergelow=int(1.5*$Nmerge*$low);
		if($sup1 <= $mergelow or $sup2 <= $mergelow){
			push @q,"LowQual1";
		}
		
		my $mergelows=(int(($Nmerge**2)*0.4)>1 ? int(($Nmerge**2)*0.4):1)*$lows;
		if($sup3 <= $mergelows){
			push @q,"LowQual2";
		}
		...
		print OUTA join("\t",@{$ref}[0,1],$qual,@{$ref}[2..10]),"\n";
