...
$line="link-id $out/$bamname.sin $out/$bamname.lnd $low $ex $blow $bex $gap $bin $Nmerge";
...
$line="judge-link $out/$bamname.sin $out/$bamname.sln $bam $phase_dir $black_list $control_list $out/$bamname.judge $bin $ncpu";
if(executeSystemCall($line)){
...
#step7
print STDERR "Step_7 link and merge SVs for final output\n";
my $merge_size=$Smerge*$bin;
$line="id_link $out/$bamname.filter $out/$bamname.sln $out/$bamname.final $merge_size $mergemax";
if(executeSystemCall($line)){
