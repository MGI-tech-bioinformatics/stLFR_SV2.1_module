...
attach(a)
r= as.numeric(a[,3])
r.center = (r < quantile(r, as.numeric(args[12])))
detach(a)
a = a[r.center, ]
...
library(ggplot2)
ggplot(a, aes(x=x, y=y, fill=Count))+geom_tile()+
geom_hline(yintercept = as.numeric(args[10]),linetype="dotted")+
geom_hline(yintercept = as.numeric(args[11]),linetype="dotted")+
geom_vline(xintercept = as.numeric(args[8]),linetype="dotted")+
geom_vline(xintercept = as.numeric(args[9]),linetype="dotted")+
