...
use Cwd;
use FindBin qw($Bin);
die "Usage: perl $0 bam mapq bin chr1 start1 end1 chr2 start2 end2 flank prefix withchr cut\n" if @ARGV <13;

my ($bam,$mapq,$bin,$chr1,$start1,$end1,$chr2,$start2,$end2,$flank,$pre,$withchr,$cut)=@ARGV;
my (%hash1,%hash2);
my $path = $Bin;
$cut=1-$cut;
if($withchr!=0){
...
close OUT;
my $title="chr".$chr1.":".$start1."-".$end1."--"."chr".$chr2.":".$start2."-".$end2;
`Rscript $path/plot_heatmap_dif.R $pre.stat $pre $title $s1 $e1 $s2 $e2 $start1 $end1 $start2 $end2 $cut`;
