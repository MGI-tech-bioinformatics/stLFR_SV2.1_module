...
die "Usage: perl $0 bam mapq bin chr start end flank prefix withchr cut\n" if @ARGV <10;

my ($bam,$mapq,$bin,$chr,$start,$end,$flank,$pre,$withchr,$cut)=@ARGV;
my %hash;
if($withchr!=0){
	$chr="chr".$chr;
}
...
close OUT;
my $title="chr".$chr.":".$start."-".$end;
`Rscript $path/plot_heatmap.R $pre.stat $pre $title $s $e $start $end $cut`;
